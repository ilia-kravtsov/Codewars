...
function procArrInt(listNum) {
  ...
  const totalNumbers = listNum.length;
  const primes = [];
  const divisorsCountMap = new Map();
  ...
  listNum.forEach((num) => {
    ...
    const divisorsCount = getDivisorsCount(num);
    if (divisorsCountMap.has(divisorsCount)) {
      divisorsCountMap.get(divisorsCount).push(num);
    } else {
      divisorsCountMap.set(divisorsCount, [num]);
    }
  });

  const maxDivisorsCount = Math.max(...divisorsCountMap.keys());
  const numbersWithMaxDivisors = divisorsCountMap.get(maxDivisorsCount).sort((a, b) => a - b);

  return [totalNumbers, primes.length, [maxDivisorsCount, numbersWithMaxDivisors]];
}