...
function encodeResistorColors(ohmsString) {
  ...
  let [value, unit] = ohmsString.split(" ");
  let multiplier = 0;
  if (value.includes("k")) {
  ...
  const significantDigits = Math.floor(value).toString().slice(0, 2);
  multiplier = Math.floor(Math.log10(value)) - (significantDigits.length - 1);
  const firstDigit = colorCodes[parseInt(significantDigits[0])];
  const secondDigit = colorCodes[parseInt(significantDigits[1])];
  const multiplierColor = colorCodes[multiplier];
  const tolerance = "gold";
  return `${firstDigit} ${secondDigit} ${multiplierColor} ${tolerance}`;
}