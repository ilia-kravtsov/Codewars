...
DI.prototype.inject = function (func) {
  var self = this;
  var argNames = func.toString().match(/^function\s*[^\(]*\(\s*([^\)]*)\)/m)[1].split(',').map(function(argName) {
    return argName.trim();
  });
  var dependencies = argNames.map(function(argName) {
    return self.dependency[argName];
  }).filter(function(dep) {
