...
function tongues(code) {
    const vowels = 'aiyeouAIYEOU';
    const consonants = 'bkxznhdcwgpvjqtsrlmfBKXZNHDCWGPVJQTSRLMF';
    function transformChar(char) {
        if (vowels.includes(char)) {
            const index = vowels.indexOf(char);
            return vowels[(index + 3) % 6 + (index >= 6 ? 6 : 0)];
        } else if (consonants.includes(char)) {
            const index = consonants.indexOf(char);
            return consonants[(index + 10) % 20 + (index >= 20 ? 20 : 0)];
        } else {
            return char;
        }
    }
    return Array.from(code).map(transformChar).join('');
}
