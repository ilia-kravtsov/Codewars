...
function workOnStrings(a, b) {
  ...
  function countOccurrences(str, char) {
    ...
    return str.split("").filter((c) => c.toLowerCase() === lowerChar).length;
  }
  function applyCaseSwapping(source, target) {
    let result = target.split("");
    for (let char of source) {
      const occurrences = countOccurrences(source, char);
      for (let i = 0; i < result.length; i++) {
        if (result[i].toLowerCase() === char.toLowerCase()) {
          for (let j = 0; j < occurrences; j++) {
            result[i] = swapCase(result[i]);
          }
        }
      }
    }
  ...
  const modifiedA = applyCaseSwapping(b, a);
  const modifiedB = applyCaseSwapping(a, b);
