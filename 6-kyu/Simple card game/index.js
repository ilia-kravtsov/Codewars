...
function winner(deckSteve, deckJosh) {
  ...
  for (let i = 0; i < deckSteve.length; i++) {
      ...
      const isLetterSteve = /[a-zA-Z]/.test(deckSteve[i])
      const isLetterJosh = /[a-zA-Z]/.test(deckJosh[i])
      
      if (isLetterSteve && isLetterJosh) {
          
          if (values[deckSteve[i]] > values[deckJosh[i]]) {
            
              steveScore++
          } else if (values[deckJosh[i]] > values[deckSteve[i]]) {
              joshScore++
          }
      } else if (isLetterSteve || isLetterJosh) {
          if (isLetterSteve) {
              if (values[deckSteve[i]] > deckJosh[i]) {
                  steveScore++
              } else if (deckJosh[i] > values[deckSteve[i]]) {
                  joshScore++
              }
          } else if (isLetterJosh) {
              if (values[deckJosh[i]] > deckSteve[i]) {
                  joshScore++
              } else if (deckSteve[i] > values[deckJosh[i]]) {
                  steveScore++
              }
          }
      } else {
          if (deckSteve[i] > deckJosh[i]) {
              steveScore++
          } else if (deckJosh[i] > deckSteve[i]) {
              joshScore++
          }
      }
  }

  return steveScore > joshScore ? `Steve wins ${steveScore} to ${joshScore}` : joshScore > steveScore ? `Josh wins ${joshScore} to ${steveScore}` : 'Tie'
}