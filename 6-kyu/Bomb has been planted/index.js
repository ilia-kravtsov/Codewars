...
function bombHasBeenPlanted(map, time) {
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      if (map[i][j] === "CT") ctPosition = [i, j];
      if (map[i][j] === "B") bombPosition = [i, j];
      if (map[i][j] === "K") kitPosition = [i, j];
    }
  }
  ...
  const distToBomb = bfs(ctPosition, bombPosition);
  const distToKit = kitPosition ? bfs(ctPosition, kitPosition) : Infinity;
  const distFromKitToBomb = kitPosition
    ? bfs(kitPosition, bombPosition)
    : Infinity;

  if (distToBomb + 10 <= time) {
    return true;
  } else if (distToKit + distFromKitToBomb + 5 <= time) {
    return true;
  }
