...
function findHobbitsIn(manifest) {
  const hobbitKeywords = ['halfling', 'hobbit', 'shire', 'farthing', 'hobbiton'];

  const isHobbit = (entry) => {
    const race = entry.race ? entry.race.toLowerCase() : '';
    const home = entry.home ? entry.home.toLowerCase() : '';
    const combinedFields = `${race} ${home}`;

    return hobbitKeywords.some(keyword => combinedFields.includes(keyword));
  };

  return manifest.filter(isHobbit);
}