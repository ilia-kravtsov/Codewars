...
let hypertensive = function (patients) {
  ...
  patients.forEach((patient) => {
    let systolicSum = 0;
    let diastolicSum = 0;
    let numMeasurements = patient.length;
    let hasSevereHypertension = false;

    patient.forEach((measurement) => {
      let [systolic, diastolic] = measurement.split("/").map(Number);

      systolicSum += systolic;
      diastolicSum += diastolic;

      if (systolic >= 180 && diastolic >= 110) {
        hasSevereHypertension = true;
      }
    });

    let avgSystolic = systolicSum / numMeasurements;
    let avgDiastolic = diastolicSum / numMeasurements;

    if (
      hasSevereHypertension ||
      (numMeasurements >= 2 && avgSystolic >= 140) ||
      (numMeasurements >= 2 && avgDiastolic >= 90)
    ) {
      hypertensiveCount++;
    }
  });

  return hypertensiveCount;
};
