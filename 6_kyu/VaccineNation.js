...
function vaccineList(age, status, month) {
  ...
  const addVaccinesForAge = (age) => {
    for (const [vaccine, ages] of Object.entries(vaccines)) {
  ...
  addVaccinesForAge(age);

  if (status !== "up-to-date") {
    addVaccinesForAge(status);
  }

  const fluMonths = ["september", "october", "november"];
  if (typeof month === "string" && fluMonths.includes(month.toLowerCase())) {
