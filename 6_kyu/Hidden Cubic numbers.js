...
function isSumOfCubes(s) {
  function isCubic(num) {
    ...
    const sumOfCubes = digits.reduce(
      (sum, digit) => sum + Math.pow(Number(digit), 3),
      0
    );
  ...
  if (matches) {
    for (let match of matches) {
      for (let i = 0; i < match.length; i += 3) {
        const group = match.slice(i, i + 3);
        const num = Number(group);
        if (num <= 999 && isCubic(num)) {
          cubicNumbers.push(num);
        }
      }
    }
  }
  ...
  if (cubicNumbers.length > 0) {
    const sum = cubicNumbers.reduce((total, num) => total + num, 0);
