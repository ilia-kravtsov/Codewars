...
function distribute(nodes, workload) {
  const baseJobsPerNode = Math.floor(workload / nodes);
  const extraJobs = workload % nodes;
  
  const distribution = [];
  let currentJob = 0;
  
  for (let i = 0; i < nodes; i++) {
    const jobsForThisNode = baseJobsPerNode + (i < extraJobs ? 1 : 0);
    const jobs = [];
    
    for (let j = 0; j < jobsForThisNode; j++) {
      jobs.push(currentJob++);
    }
    
    distribution.push(jobs);
  }
