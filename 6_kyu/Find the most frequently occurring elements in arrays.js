...
function getMostFrequent(json) {
  return json.temperature.map((dayArray) => {
    const frequencyMap = new Map();

    dayArray.forEach((temp, index) => {
      frequencyMap.set(temp, (frequencyMap.get(temp) || 0) + 1);
    });

    let mostFrequent = dayArray[0];
    let maxFrequency = 0;

    dayArray.forEach((temp) => {
      const freq = frequencyMap.get(temp);
      if (
        freq > maxFrequency ||
        (freq === maxFrequency &&
          dayArray.lastIndexOf(temp) > dayArray.lastIndexOf(mostFrequent))
      ) {
        mostFrequent = temp;
        maxFrequency = freq;
      }
