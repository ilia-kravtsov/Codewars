...
function getCellAddresses(range) {
  ...
  const startColNumber = columnLetterToNumber(startCol);
  const endColNumber = columnLetterToNumber(endCol);

  if (
    startRow > endRow ||
    startColNumber > endColNumber ||
    (startRow === endRow && startColNumber === endColNumber)
  ) {
    return [];
  }
  ...
  for (let row = startRow; row <= endRow; row++) {
    for (let col = startColNumber; col <= endColNumber; col++) {
      result.push(columnNumberToLetter(col) + row);
    }
  }
