...
function shoot(x) {
  ...
  for (let i = 0; i < x.length; i++) {
    let round = x[i][0];
    let doublePoints = x[i][1];

    if (doublePoints) {
      peteScore += (round.P1.split("X").length - 1) * 2;
      philScore += (round.P2.split("X").length - 1) * 2;
    } else {
      peteScore += round.P1.split("X").length - 1;
      philScore += round.P2.split("X").length - 1;
    }
  }
