...
function getColors(colArr) {
  ...
  return colArr.map(subArr => {
    const colorCounts = [0, 0, 0]; 

    subArr.forEach(hexColor => {
      const rgb = hexColor.match(/.{1,2}/g).map(hex => parseInt(hex, 16)); 
      const maxIndex = rgb.indexOf(Math.max(...rgb)); 
      colorCounts[maxIndex]++;
    });

    const majorIndex = colorCounts.indexOf(Math.max(...colorCounts)); 
    colorCounts[majorIndex] = -Infinity; 
    const minorIndex = colorCounts.indexOf(Math.max(...colorCounts)); 

    return `${colorNames[majorIndex]}+${colorNames[minorIndex]}`; 
  }).join(',');
}